...
describe("PostgreSQL repositories with a controlled SQL adapter", () => {
  test("builds every repository and exercises item filtering and ordering", async () => {
    ...
    expect(Object.keys(repos)).toEqual([
      "items", "captures", "captureTokens", "rateLimits", "oauthTokens", "summaries",
      "feedback", "research", "settings", "notifications", "embeddings", "rawContent",
      "publisherQueue", "jobs", "agent",
    ]);

    await expect(repos.items.list()).resolves.toHaveLength(1);
    await expect(repos.items.list({ sort: "priority", includeProcessing: true })).resolves.toHaveLength(1);
    await expect(repos.items.list({ sort: "ai_priority", sourceType: "manual" })).resolves.toHaveLength(1);
    await expect(
      repos.items.list({
  ...
  test("handles item insert, update, delete, and status mutations", async () => {
    const fake = sqlDouble([
      [{ id: "item-1" }], [itemRow],
      [],
      [itemRow], [], [{ ...itemRow, title: "Changed" }],
      [],
      [{ id: "item-1" }], [], [],
    ]);
    const items = createPostgresRepositories(fake.sql).items;
    const input = {
      id: "item-1", title: "Article", summary: "Summary", sourceType: "manual" as const,
      contentType: "article" as const, topics: ["testing"], url: "https://example.com/article?utm_source=x",
      priority: "medium" as const, isRead: false, createdAt: "2026-01-01T00:00:00Z",
    };
    ...
    await expect(items.update("item-1", { title: "Changed" })).resolves.toMatchObject({ title: "Changed" });
  ...
  test("maps capture records and enforces an empty transition precondition", async () => {
    const fake = sqlDouble([
      [captureRow], [captureRow], [], [captureRow], [captureRow], [],
    ]);
    const captures = createPostgresRepositories(fake.sql).captures;
    const created = await captures.create({
      id: "capture-1", url: "https://example.com/article", normalizedUrl: "https://example.com/article",
      topics: [], priority: "medium", source: "web", createdAt: "2026-01-01T00:00:00Z",
    });
    ...
    await expect(captures.findActiveOrReadyByNormalizedUrl("https://example.com/article")).resolves.toMatchObject({ id: "capture-1" });
    await expect(captures.list()).resolves.toHaveLength(1);
    const callsBefore = (fake.sql as unknown as jest.Mock).mock.calls.length;
    await expect(captures.transition("capture-1", [], { status: "ready", updatedAt: "now" })).resolves.toBeUndefined();
    expect((fake.sql as unknown as jest.Mock).mock.calls).toHaveLength(callsBefore);
    await expect(captures.transition("capture-1", ["queued"], { status: "processing", updatedAt: "now" })).resolves.toBeUndefined();
  });

  test("covers token lifecycle and deterministic rate-limit windows", async () => {
    const tokenRow = {
      id: "token-1", name: "Phone", token_hash: "hash", token_prefix: "dst_cap_123",
      created_at: new Date("2026-01-01T00:00:00Z"), last_used_at: null, revoked_at: null,
    };
    const fake = sqlDouble([[], [tokenRow], [], [tokenRow], [{ id: "token-1" }], [], [], [{ count: 1 }], [{ count: 3 }]]);
    const repos = createPostgresRepositories(fake.sql);
    await repos.captureTokens.create({
      id: "token-1", name: "Phone", tokenHash: "hash", tokenPrefix: "dst_cap_123",
      createdAt: "2026-01-01T00:00:00Z",
    });
    await expect(repos.captureTokens.findActiveByHash("hash")).resolves.toMatchObject({ id: "token-1" });
    ...
    await expect(repos.rateLimits.consume({ key: "ip", limit: 2, windowSeconds: 60, now: "2026-01-01T00:00:59Z" })).resolves.toEqual({
      allowed: true, remaining: 1, resetAt: "2026-01-01T00:01:00.000Z",
    });
    await expect(repos.rateLimits.consume({ key: "ip", limit: 2, windowSeconds: 60, now: "2026-01-01T00:00:59Z" })).resolves.toEqual({
      allowed: false, remaining: 0, resetAt: "2026-01-01T00:01:00.000Z",
    });
  });

  test("supports OAuth lookup branches and nullable field mapping", async () => {
    const row = {
      provider: "slack", team_id: "team", access_token: "access", refresh_token: null,
      expiry_date: null, email: null, updated_at: "2026-01-01T00:00:00Z",
    };
    const fake = sqlDouble([[row], [row], [], [row], [], [], []]);
    const oauth = createPostgresRepositories(fake.sql).oauthTokens;
    await expect(oauth.find("slack")).resolves.toMatchObject({ provider: "slack", refreshToken: undefined });
    ...
    expect(fake.queries.filter((query) => query.includes("DELETE FROM oauth_tokens"))).toHaveLength(2);
  });
  ...
  test("covers durable supporting repositories and their result shaping", async () => {
    ...
    respond({ id: "n", item_id: "item-1", title: "Ready", message: "Done", is_read: false, created_at: "2026-01-01Z" });
    await expect(r.notifications.list()).resolves.toEqual([expect.objectContaining({ id: "n", isRead: false })]);
    respond({ count: 2 });
    ...
    await expect(r.embeddings.find("item-1")).resolves.toMatchObject({ itemId: "item-1", embedding: [0.1] });
    ...
    await expect(r.embeddings.listRecent()).resolves.toEqual([{ itemId: "item-1", embedding: [0.2] }]);

    await r.rawContent.insert({ id: "raw", sourceType: "manual", rawBody: "body", metadata: {}, fetchedAt: "now" });
    await r.rawContent.attachItem("raw", "item-1");
    await r.publisherQueue.enqueue("pub", "https://example.com");
    respond({ publisher_id: "pub", url: "https://example.com", discovered_at: "2026-01-01Z", status: "pending", attempts: 1, last_error: null });
    await expect(r.publisherQueue.listPending("pub")).resolves.toEqual([expect.objectContaining({ publisherId: "pub", lastError: undefined })]);
    await r.publisherQueue.markFetched("pub", "https://example.com");
    await r.publisherQueue.markFailed("pub", "https://example.com", "timeout");
    respond(
      { status: "pending", count: 1 }, { status: "fetched", count: 2 },
      { status: "failed", count: 3 }, { status: "unknown", count: 99 }
    );
    await expect(r.publisherQueue.getStats("pub")).resolves.toEqual({ pending: 1, fetched: 2, failed: 3 });
    ...
    respond({ status: "pending", count: 1 }, { status: "completed", count: 2 }, { status: "other", count: 8 });
    await expect(r.jobs.getStats()).resolves.toEqual({ pending: 1, running: 0, completed: 2, failed: 0 });
  });
  ...
  test("covers agent persistence parsing and workflow branches", async () => {
    ...
    await expect(agent.getDailyAuditStats()).resolves.toEqual({ totalCost: 1, totalCalls: 2, totalTokens: 3 });
    await expect(agent.getDailyAuditStats()).resolves.toEqual({ totalCost: 0, totalCalls: 0, totalTokens: 0 });
    ...
    await agent.insertApproval({ id: "ap1", actionType: "publish", description: "Publish", payload: '{"ok":true}' });
    await agent.insertApproval({ id: "ap2", actionType: "publish", description: "Publish", payload: "bad-json" });
    await agent.listPendingApprovals();
    await agent.resolveApproval("ap1", "approved");
    await agent.insertConversation({ id: "c" });
    await agent.insertMessage({ id: "m1", conversationId: "c", role: "user", content: "hello", citations: '[{"id":1}]', toolCalls: { name: "save" } });
    await agent.insertMessage({ id: "m2", conversationId: "c", role: "assistant", content: "hello", citations: "plain", toolCalls: null });
  ...
  test("maps summary, feedback, and research records including missing-row branches", async () => {
    ...
    const r = createPostgresRepositories(fake.sql);
    const respond = (...rows: unknown[]) => fake.responses.push(rows);
    const summary = { id: "s", item_id: "item-1", summary: "Brief", model: "model", prompt_type: "brief", created_at: "2026-01-01Z" };
    respond(summary);
    ...
    await expect(r.summaries.findAll("item-1")).resolves.toEqual({ brief: "Brief", detailed: "Detailed" });
    respond(summary);
    await expect(r.summaries.upsert({ id: "s", itemId: "item-1", summary: "Brief", model: "model", promptType: "brief" })).resolves.toMatchObject({ id: "s" });
    await r.summaries.deleteForItem("item-1");

    const feedback = { id: "f", item_id: "item-1", rating: 1, reason: null, created_at: "2026-01-01Z" };
    respond(feedback);
    await expect(r.feedback.insert({ id: "f", itemId: "item-1", rating: 1 })).resolves.toMatchObject({ reason: undefined });
    ...
    const report = {
      id: "report", item_id: null, query: "testing", report: "body", sources: "[]", model: "model",
      status: "pending", created_at: "2026-01-01Z", completed_at: null, progress: null,
    };
    respond(report);
    await expect(r.research.insertReport({ id: "report", query: "testing", model: "model" })).resolves.toMatchObject({ itemId: undefined });
    respond({ ...report, item_id: "item-1", completed_at: "2026-01-02Z", progress: "done" });
    await expect(r.research.findReport("report")).resolves.toMatchObject({ itemId: "item-1", progress: "done" });
    ...
    await expect(r.research.updateReport("report", { status: "complete", progress: "done" })).resolves.toMatchObject({ status: "complete" });
    respond(report);
    await expect(r.research.listReports()).resolves.toHaveLength(1);

    const suggestion = {
      id: "sg", topic_key: "testing", topic: "Testing", reason: "Useful",
      suggested_query: "testing", source_item_ids: ["item-1"], status: "pending",
      research_report_id: null, created_at: "2026-01-01Z",
    };
    respond(suggestion);
    await expect(r.research.listPendingSuggestions()).resolves.toHaveLength(1);
    respond({ ...suggestion, research_report_id: "report" });
    await expect(r.research.findSuggestion("sg")).resolves.toMatchObject({ researchReportId: "report" });
    await expect(r.research.findSuggestion("missing")).resolves.toBeUndefined();
    await r.research.replacePendingSuggestions([
      { id: "sg", topicKey: "testing", topic: "Testing", reason: "Useful", suggestedQuery: "testing", sourceItemIds: ["item-1"] },
    ]);
    respond({ id: "sg" });
