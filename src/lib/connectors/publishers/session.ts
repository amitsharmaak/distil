...
async function clearStaleLocks(dir: string): Promise<void> {
  for (const name of ["SingletonLock", "SingletonCookie", "SingletonSocket"]) {
    // This path is local-only connector state. Do not make Turbopack trace the
    // entire application when publisher connectors are bundled but disabled.
    await unlink(path.join(/* turbopackIgnore: true */ dir, name)).catch(() => undefined);
  }
}
