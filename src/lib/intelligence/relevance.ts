...
import type { RawContent, ContentClassification, RelevanceResult, EmailCategory } from "./types";
 ...
 */
function inferEmailCategoryFromGmailSignals(raw: RawContent): EmailCategory | null {
...
const DEFAULT_ALLOWED_CATEGORIES: EmailCategory[] = ["newsletter", "digest", "announcement"];

const EMAIL_INTELLIGENCE_KEY = "email_intelligence_categories";

function parseAllowedCategories(value: string | undefined): EmailCategory[] {
  ...
  try {
    ...
    const filtered = parsed.filter(
      (c): c is EmailCategory => typeof c === "string" && valid.has(c)
    );
...
export async function checkRelevance(
  ...
  getUserSetting: (key: string) => Promise<string | undefined>
  ...
  const allowedCategories = parseAllowedCategories(await getUserSetting(EMAIL_INTELLIGENCE_KEY));
