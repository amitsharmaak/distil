...
const runBatch = (payload: Record<string, unknown>, dependencies: KnowledgeBackfillDependencies) =>
  runKnowledgeBackfillBatch(
    context,
    { userId: context.userId, traceId: context.requestId, ...payload },
    dependencies
  );
...
describe("knowledge backfill jobs", () => {
  ...
  it("does not enqueue an already completed checkpoint", async () => {
    ...
    await enqueueKnowledgeBackfill({ context, repositories: deps, kind: "content_versions", now });
  ...
  it("chunks versions with source offsets and no configured embeddings", async () => {
    ...
    await runBatch({ jobKey: "kbf_job", kind: "chunks", batchSize: 5 }, deps);
  ...
  it("imports legacy summaries without verification and generates explicit fallbacks", async () => {
    ...
    await runBatch({ jobKey: "kbf_job", kind: "legacy_artifacts", batchSize: 5 }, legacyDeps);
    ...
    await runBatch({ jobKey: "kbf_job", kind: "degraded_summaries", batchSize: 5 }, degradedDeps);
  ...
  it("fails the durable checkpoint when a batch write fails", async () => {
    ...
    await expect(
      runBatch({ jobKey: "kbf_job", kind: "content_versions", batchSize: 5 }, deps)
  ...
  it("validates batch bounds before touching persistence", async () => {
    const deps = dependencies();
    await expect(
      runBatch({ jobKey: "kbf_job", kind: "content_versions", batchSize: 101 }, deps)
  ...
  it("rejects missing checkpoints and mismatched checkpoint types", async () => {
    ...
    await expect(
      runBatch({ jobKey: "missing", kind: "content_versions", batchSize: 1 }, missing)
    ).rejects.toThrow("does not exist");

    const mismatched = dependencies(checkpoint({ jobType: "chunks" }));
    await expect(
      runBatch({ jobKey: "kbf_job", kind: "content_versions", batchSize: 1 }, mismatched)
  ...
  it("returns completed or concurrently unavailable checkpoints without running a batch", async () => {
    ...
    await expect(
      runBatch({ jobKey: "kbf_job", kind: "content_versions", batchSize: 1 }, completedDeps)
    ...
    await expect(
      runBatch({ jobKey: "kbf_job", kind: "content_versions", batchSize: 1 }, unavailableDeps)
